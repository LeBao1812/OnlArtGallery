...
namespace ArtGallery.Controllers
{
    public class HomeController : Controller
    {
                ...
                using (var ctx = new ArtGalleryEntities())
                {
                    foreach (var item in cv)
                    {
                        ctx.MessageLists.Add(new MessageList
                        {
                            ...
                            ContentM = "I have successfully paid for the painting, please deliver it to me quickly, you can see my address and phonenumber on my profile.",
                            IDUS = idu,
                            IDUR = ctx.Arts.Where(a => a.IDA == item.IDA).Select(a => a.IDU).FirstOrDefault(),
                            StatusM = false,
                        });
                        var exc = ctx.Carts.Where(c => c.IDA == item.IDA);
                        ctx.Carts.RemoveRange(exc);
        ...
        public ActionResult UpdatePriceAuction(int ida = 0, int price = 0)
        {
            ...
            {
                int highestprice = 0;
                int excount = ctx.Auctionhistories.Where(a => a.IDA == ida).OrderByDescending(a => a.PriceAu).Count();
                if (excount > 0)
                {
                    var ex = ctx.Auctionhistories.Where(a => a.IDA == ida).OrderByDescending(a => a.PriceAu).FirstOrDefault();
                    highestprice = Convert.ToInt32(ex.PriceAu);
                }
                else {
                    highestprice =Convert.ToInt32(ctx.Arts.Where(a => a.IDA == ida).Select(a => a.PriceA).FirstOrDefault());
                }
                ...
                if (highestprice < price)
                {
                    if (price < 200000000)
                    {
                        ...
                        return Json(new { success = false, message = "Max price is 2000000000!" }, JsonRequestBehavior.AllowGet);
                    }
